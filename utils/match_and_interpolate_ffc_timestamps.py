...
converted_ffc_poses = pd.read_csv("V6_Updated_PF_Absolute_FFC_Pose_Quat_RFCTimestamps.csv")

# Load the ground truth FFC poses from CSV (these have ground truth FFC timestamps)
ground_truth_ffc_poses = pd.read_csv('Data\cleaned_FFC_timestamps.csv')
...
# Copy the first pose directly from the converted FFC poses
first_pose = converted_ffc_poses.iloc[0]
first_pose_timestamp = ground_truth_ffc_poses.iloc[0]['pose_timestamp']
...
# Iterate over each timestamp in the ground truth FFC poses starting from pose 2
for i in range(1, len(ground_truth_ffc_poses['pose_timestamp'])):  # Start from the second pose
    #if i >= 1640:  # We are only interested in the first 1640 ground truth FFC timestamps
        #break
    
    target_timestamp = ground_truth_ffc_poses.iloc[i]['pose_timestamp']
    
    # Find the two closest timestamps in the converted FFC poses (with RFC timestamps)
    closest_index, second_closest_index, weight = find_closest_timestamps_and_weight(target_timestamp, converted_ffc_poses['pose_timestamp'])
    
    # Get the two closest poses
    pose1 = converted_ffc_poses.iloc[closest_index][['tx', 'ty', 'tz', 'ox', 'oy', 'oz', 'ow']].to_numpy()
    pose2 = converted_ffc_poses.iloc[second_closest_index][['tx', 'ty', 'tz', 'ox', 'oy', 'oz', 'ow']].to_numpy()
    
    # Interpolate to get the estimated FFC pose
    interpolated_pose = interpolate_poses(pose1, pose2, weight)
    
    # Append the interpolated pose to the final DataFrame with the ground truth FFC timestamp
    final_ffc_poses_list.append({
        'pose_timestamp': ground_truth_ffc_poses.iloc[i]['pose_timestamp'],
...
final_ffc_poses = pd.concat([pd.DataFrame([pose]) for pose in final_ffc_poses_list], ignore_index=True)

# Save the final FFC poses with matched timestamps to a new CSV file
final_ffc_poses.to_csv("V6_Updated_PF_Absolute_FFC_Pose_Quat.csv", index=False)

print("Done matching timestamps and interpolating final FFC pose")
